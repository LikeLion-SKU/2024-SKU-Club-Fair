import likelion from './public/likelion.jpg';
import test1 from './public/test1.jpg';
import map1 from './public/map/1.svg';
import map2 from './public/map/2.svg';
import map3 from './public/map/3.svg';
import map4 from './public/map/4.svg';
import map5 from './public/map/5.svg';
import map6 from './public/map/6.svg';
import map7 from './public/map/7.svg';
import map8 from './public/map/8.svg';
import map9 from './public/map/9.svg';
import map10 from './public/map/10.svg';
import map11 from './public/map/11.svg';
import map12 from './public/map/12.svg';
import map13 from './public/map/13.svg';
import map14 from './public/map/14.svg';
import map15 from './public/map/15.svg';

export const clubs = [
  {
    id: '1',
    title: '멋쟁이사자처럼',
    type: '학술',
    img: likelion,
    description:
      '나만의 웹페이지를 만들고 싶거나, 상상만 했던 아이디어를 현실로 실현 할 수 있는 전국 최대 규모 IT창업 동아리 입니다',
    activity:
      '매주 1회 필참 파트별 트랙, 자율 스터디, 아이디어톤, 해커톤, 데모데이, 연합해커톤 등등 다양한 IT 주요활동을 경험 할 수 있습니다.',
    representatives: [
      {
        part: '대표',
        name: '황현진',
        phone: '010-2044-3300',
      },
      {
        part: '부대표',
        name: '하경림',
        phone: '010-4353-0366',
      },
    ],
    sns: '인스타그램 (@likelion_skuniv)',
    snsLink: 'https://www.instagram.com/likelion_skuniv/',
    map: map1,
    clubinfo: {
      menu: [
        {
          name: '달고나',
          price: '3000원',
        },
        {
          name: '사탕',
          price: '3000원',
        },
      ],
      game: [
        {
          name: '동아리 유형 테스트,',
          price: '0원',
        },
        {
          name: '운영진과 타자대결,',
          price: '0원',
        },
        {
          name: '그림 퀴즈',
          price: '0원',
        },
      ],
    },
  },
  {
    id: '2',
    title: 'CCC',
    type: '종교',
    img: test1,
    description:
      'Campus Cursade for Christ의 약어로 한국 대학생 선교회이자, 기독교 동아리입니다!',
    activity:
      '1 대 1 맨토링, 가족순모임, 목요 채플, 여름/겨울 수련회, 단기선교 등이 있습니다.',
    representatives: [
      {
        part: '대표',
        name: '김준영',
        phone: '010-5392-6208',
      },
      {
        part: '간사',
        name: '이현우',
        phone: '010-5027-0288',
      },
    ],
    sns: '인스타그램 (@sku_ccc)',
    snsLink: 'https://www.instagram.com/sku_ccc/',
    map: map5,
    clubinfo: {
      menu: [
        {
          name: '팝콘',
          price: '무료',
        },
      ],
      game: [
        {
          name: '더 포 설문조사',
          price: '0원',
        },
      ],
    },
  },
  {
    id: '3',
    title: '적시타',
    type: '체육',
    img: test1,
    description:
      '야구를 좋아한다면 혹은 다양한 학과 사람들과 낭만있는 대학생활을 즐기고 싶다면 누구든지 적시타의 문을 두드려라! 야구를 몰라도 장비가 없어도 "열정"만 있다면 누구나, 언제든 두팔벌려 환영합니다.',
    activity:
      'AUBL(아마추어수도권대학야구리그)참가, 친선경기, 훈련, 프로야구 직관, 소모임, OB와 교류, 각종 파티 및 회식',
    representatives: [
      {
        part: '대표',
        name: '이찬호',
        phone: '010-4184-6214',
      },
      {
        part: '부대표',
        name: '강민교',
        phone: '010-9039-5672',
      },
    ],
    sns: '인스타그램 (@juksita_2024)',
    snsLink: 'https://www.instagram.com/juksita_2024/',
    map: map12,
    clubinfo: {
      menu: [
        {
          name: '소세지',
          price: '3000원',
        },
        {
          name: '소떡소떡',
          price: '3000원',
        },
      ],
      game: [
        {
          name: '페펙트 피쳐,',
          price: '1000원',
        },
        {
          name: '스피드킹',
          price: '1000원',
        },
      ],
    },
  },
  {
    id: '4',
    title: '한국기독학생회IVF',
    type: '종교',
    img: test1,
    description:
      '한국기독학생회 IVF는 북서울 11 개 캠퍼스, 전국 150여 곳의 대학생과  활동하는 대학생 선교단체로, 대학생들이 그리스도의 제자로서  살도록 격려하는 공동체입니다.',
    activity:
      '소그룹모임, 북서울 지방 대학교 연합MT 및 모임, 성균관-서경 지부 모임, 교내활동, 수련회  ',
    representatives: [
      {
        part: '대표',
        name: '이요한',
        phone: '010-6551-7686',
      },
      {
        part: '부대표',
        name: '유태영',
        phone: '010-9552-7290',
      },
    ],
    sns: '인스타그램 (@sku_ivf)',
    snsLink: 'https://www.instagram.com/sku_ivf/',
    map: map9,
    clubinfo: {
      menu: [
        {
          name: '어묵',
          price: '무료',
        },
      ],
      game: [
        {
          name: 'Q&A세션',
          price: '0원',
        },
      ],
    },
  },
  {
    id: '5',
    title: '핀쓰리',
    type: '체육',
    img: test1,
    description:
      '저희 핀쓰리는 유일한 볼링동아리로 볼링 실력 향상과 다양한 사람들과의 친목을 목적으로 하는 동아리입니다.',
    activity:
      '정기전(매주 금요일), 타학교와의 교류전, 이벤트전(1학기당 최소 1번 진행), 번개 모임, 볼린이 프로젝트(볼링 강습) 등',
    representatives: [
      {
        part: '대표',
        name: '박성지',
        phone: '010-3219-5246',
      },
      {
        part: '부대표',
        name: '노희원',
        phone: '010-6674-8083',
      },
    ],
    sns: '인스타그램 (@sku_pinthree_24)',
    snsLink: 'https://www.instagram.com/sku_pinthree_24/',
    map: map6,
    clubinfo: {
      menu: [
        {
          name: '없음',
          price: '없음',
        },
      ],
      game: [
        {
          name: '미니 볼링 게임,',
          price: '0원',
        },
        {
          name: '스플릿 메이드 도전,',
          price: '0원',
        },
        {
          name: '볼링 퀴즈,',
          price: '0원',
        },
        {
          name: '볼링 용품 전시',
          price: '0원',
        },
      ],
    },
  },
  {
    id: '6',
    title: 'UBF',
    type: '종교',
    img: test1,
    description:
      'University Bible Fellowship 대학생 성경읽기 선교회로 이 시대를 빚어가는 청년들이 말씀을 통해 자신의 정체성을 발견하고 비전을 전하기 위한 선교단체입니다.',
    activity:
      '바이블 아카데미, 1 대 1 멘토링, 다양한 취미생활 공유, 여름/겨울 수련회',
    representatives: [
      {
        part: '대표',
        name: '신유진',
        phone: '010-8030-7469',
      },
      {
        part: '간사',
        name: '안창현',
        phone: '010-8003-5872',
      },
    ],
    sns: '인스타그램 (@sk_ubf)',
    snsLink: 'https://www.instagram.com/sk_ubf/',
    map: map13,
    clubinfo: {
      menu: [
        {
          name: '달고나',
          price: '무료',
        },
      ],
      game: [
        {
          name: '달고나 및 비즈 만들기,',
          price: '0원',
        },
        {
          name: '설문조사',
          price: '0원',
        },
      ],
    },
  },
  {
    id: '7',
    title: 'WORKERS',
    type: '공연예술',
    img: test1,
    description: `안녕하세요 서경대학교 중앙 밴드 동아리 워커스입니다!
워커스와 함께 대학 생활의 청춘과 낭만을 즐기고 싶으신 분들은
망설이지 말고 지원해주세요:)`,
    activity: `주요 활동으로는 축제, 동아리 알림제 등 학교 행사 공연과 1년마다 하는 동아리 정기 공연, 타대학 밴드와의 연합 공연이 있습니다. 공연 외에도 MT나 회식을 정기적으로 진행하여 부원들간의 다양한 친목 활동도 이루어집니다.`,
    representatives: [
      {
        part: '대표',
        name: '신윤아',
        phone: '010-3627-9246',
      },
      {
        part: '부대표',
        name: '김예진',
        phone: '010-4424-4783',
      },
    ],
    sns: '인스타그램 (@sku_workers)',
    snsLink: 'https://www.instagram.com/sku_workers/',
    map: map3,
    clubinfo: {
      menu: [
        {
          name: '메론소다',
          price: '2500원',
        },
        {
          name: '츄러스',
          price: '3000원',
        },
      ],
      game: [
        {
          name: '전주 1초 듣고 노래 맞히기,',
          price: '0원',
        },
        {
          name: '피아노 음계 맞히기',
          price: '0원',
        },
      ],
    },
  },
  {
    id: '8',
    title: '민형사법연구회',
    type: '학술',
    img: test1,
    description: `법에 관심있는 그대, 민형사법연구회와 함께`,
    activity: `법률탐구, 법률포럼참석, 법률기관탐방, 그룹스터디 그외 다양한 친목활동(MT, 번개 등)`,
    representatives: [
      {
        part: '홍보국장',
        name: '이태웅',
        phone: '010-7132-1779',
      },
      {
        part: '대표',
        name: '김요한',
        phone: '010-4264-1619',
      },
    ],
    sns: '인스타그램 (@sku.cclrs)',
    snsLink: 'https://www.instagram.com/',
    map: map15,
    clubinfo: {
      menu: [
        {
          name: '컵과자',
          price: '0원',
        },
        {
          name: '탄산음료',
          price: '0원',
        },
        {
          name: '식혜',
          price: '0원',
        },
        {
          name: '하이볼',
          price: '0원',
        },
      ],
      game: [
        {
          name: '법률 퀴즈,',
          price: '0원',
        },
        {
          name: '민사 소개팅',
          price: '0원',
        },
      ],
    },
  },
  {
    id: '9',
    title: '애니문',
    type: '전시창작',
    img: test1,
    description: `애니문 동아리는 서경대 유일의 서브컬처 동아리 입니다. 활동의 결과 수준보다 활동을 통한 친목을 목적으로 모였습니다.`,
    activity: `밴드부, 그림부, 코스부등 내부 소규모 모임으로 서브컬처에 관련된 어떠한 활동도 자유롭게 참여 가능합니다.`,
    representatives: [
      {
        part: '대표',
        name: '최재현',
        phone: '010-3381-6282',
      },
      {
        part: '부대표',
        name: '이지성',
        phone: '010-8822-7349',
      },
    ],
    sns: '없음',
    snsLink: 'https://www.instagram.com/',
    map: map2,
    clubinfo: {
      menu: [
        {
          name: '없음',
          price: '0원',
        },
      ],
      game: [
        {
          name: '간단한 뽑기행사,',
          price: '0원',
        },
        {
          name: '코스 체험.',
          price: '0원',
        },
        {
          name: '운세 타로',
          price: '0원',
        },
      ],
    },
  },
  {
    id: '10',
    title: '버디버디',
    type: '체육',
    img: test1,
    description: `안녕하세요  중앙동아리 버디버디 입니다
⛳️동기부여가 필요한 분
⛳️골프를 좋아하는 분
⛳️골프를 처음 접해보는 분
⛳️라운드를 나가고 싶으신 분
⛳️골프에 관심이 있다면 누구나 환영합니다!!`,
    activity: `서로 도움을 주고 받으며 재밌고 즐겁게 골프를 즐기는 곳입니다! 골프를 접해보지 않았다해서 망설이지  않아도 됩니다 

매주 그룹을 나누어 연습을 하며 이때 부원들의 실력이 균형을 이룰 수 있도록 조를 배정 하여 연습합니다.

또한 교내 스크린 골프장을 이용하여 동아리 부원들과 게임을 하고, 추후 동아리 부원들과 함께 오크밸리CC, 엘리시안 강촌CC등 한국의 여러 명문 골프장에 가 저렴한 가격에 라운드를 할 수 있는 동아리입니다.`,
    representatives: [
      {
        part: '대표',
        name: '임휘상',
        phone: '010-8785-3737',
      },
      {
        part: '부대표',
        name: '이지성',
        phone: '010-2995-1485',
      },
    ],
    sns: '없음',
    snsLink: 'https://www.instagram.com/',
    map: map10,
    clubinfo: {
      menu: [
        {
          name: '없음',
          price: '0원',
        },
      ],
      game: [
        {
          name: '2M 퍼팅 홀컵에 넣기',
          price: '0원',
        },
      ],
    },
  },
  {
    id: '11',
    title: '그라미',
    type: '공연예술',
    img: test1,
    description: `서경대학교 중앙동아리 어쿠스틱 밴드 그라미 입니다!`,
    activity: `학교공연, 외부공연, 정기공연, 정기회식, MT!`,
    representatives: [
      {
        part: '대표',
        name: '이건',
        phone: '010-8997-1875',
      },
      {
        part: '부대표',
        name: '신희승',
        phone: '010-4099-8027',
      },
    ],
    sns: '인스타그램 (@gramy._.official)',
    snsLink: 'https://www.instagram.com/gramy._.official/',
    map: map14,
    clubinfo: {
      menu: [
        {
          name: '자몽허니블랙티',
          price: '0원',
        },
        {
          name: '청포도에이드',
          price: '0원',
        },
        {
          name: '레몬에이드',
          price: '0원',
        },
        {
          name: '바나나우유',
          price: '0원',
        },
      ],
      game: [
        {
          name: '연도별 노래 맞추기,',
          price: '0원',
        },
        {
          name: '랜덤 퀴즈,',
          price: '0원',
        },
        {
          name: '인스타 팔로우 이벤트',
          price: '0원',
        },
      ],
    },
  },
  {
    id: '12',
    title: 'OZ',
    type: '체육',
    img: test1,
    description: `서경대 보드동아리 오즈입니다!!`,
    activity: `보드연습및 강습, 뚝섬 소풍, 정기MT`,
    representatives: [
      {
        part: '대표',
        name: '최원석',
        phone: '010-5290-1650',
      },
      {
        part: '부대표',
        name: '윤정윤',
        phone: '010-2658-7156',
      },
    ],
    sns: '인스타그램 (@oz_sku)',
    snsLink: 'https://www.instagram.com/oz_sku/',
    map: map8,
    clubinfo: {
      menu: [
        {
          name: '없음',
          price: '0원',
        },
      ],
      game: [
        {
          name: '보드 대여및 강습,',
          price: '0원',
        },
        {
          name: '뽑기 이벤트,',
          price: '0원',
        },
        {
          name: '인스타 팔로우 이벤트,',
          price: '0원',
        },
        {
          name: '보드 컬링',
          price: '0원',
        },
      ],
    },
  },
  {
    id: '13',
    title: '스케쳐(Skatcher)',
    type: '전시창작',
    img: test1,
    description: `서울 및 근교의 유명 스팟을 방문하여 사진을 찍고 친목도 다지는 동아리입니다!`,
    activity: `주 1회 정기적인 출사 모임 / 친목을 위한 자유로운 번개 모임 /사진 촬영 및 보정 꿀팁 공유`,
    representatives: [
      {
        part: '대표',
        name: '임라온',
        phone: '010-8930-9117',
      },
      {
        part: '부대표',
        name: '양지수',
        phone: '010-8434-7917',
      },
    ],
    sns: '인스타그램 (@skatcher_official)',
    snsLink: 'https://www.instagram.com/skatcher_official/',
    map: map11,
    clubinfo: {
      menu: [
        {
          name: '군만두',
          price: '2000원',
        },
        {
          name: '구운 마시멜로',
          price: '2000원',
        },
      ],
      game: [
        {
          name: '무료 스냅 사진 촬영,',
          price: '0원',
        },
        {
          name: '사진 관련 퀴즈',
          price: '0원',
        },
      ],
    },
  },
];
